...
from data_validator import (DataValidationError, calculate_bearing,
                            calculate_mean_direction, calculate_mean_distance,
                            haversine_distance, validate_csv_file,
                            validate_data, validate_geographic_coordinates)
...
def load_data(catalogo_path: str = "catalogo_terremoti_unici.csv") -> pd.DataFrame:
    ...
    if catalogo_path.stat().st_size == 0:
        raise DataValidationError(
            "File catalogo e' vuoto", errors=["Catalog file is empty"]
        )
    ...
    try:
        df = validate_csv_file(
            ...
            required_columns={
                "event_id",
                "Tempo_Riferimento_ISO",
                "Numero_Stazioni_Attivate",
            },
        )
    ...
    if len(df) == 0:
        raise DataValidationError(
            "Catalogo eventi e' vuoto", errors=["No events found in catalog"]
        )
    ...
    if df.index.min() > df.index.max():
        raise DataValidationError(
            "Date nel catalogo non valide", errors=["Invalid date range"]
        )

    logger.info(
        f"✅ Caricati {len(df)} eventi unici (da {df.index.min()} a {df.index.max()})"
    )
...
def add_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for window in [6, 12, 24, 48]:
        df_orario[f"eventi_ultime_{window}h"] = (
            df_orario["numero_eventi"].rolling(window=window).sum()
        )

        df_orario[f"energia_max_ultime_{window}h"] = (
            df_orario["energia_max"].rolling(window=window).max()
        )

        df_orario[f"energia_media_ultime_{window}h"] = (
            df_orario["energia_media"].rolling(window=window).mean()
        )

        df_orario[f"energia_std_ultime_{window}h"] = (
            df_orario["energia_std"].rolling(window=window).mean()
        )
    ...
    for window in [12, 24]:
        ...
        df_orario[f"trend_eventi_{window}h"] = (
            df_orario["numero_eventi"]
        ...
        df_orario[f"trend_energia_{window}h"] = (
            df_orario["energia_max"]
            .rolling(window=window)
            .apply(
                lambda x: np.polyfit(np.arange(len(x)), x, 1)[0] if len(x) > 1 else 0,
                raw=False,
    ...
    df_orario["is_notte"] = df_orario.index.hour.isin(
        [0, 1, 2, 3, 4, 5, 22, 23]
    ).astype(int)
...
def add_future_target(
    df_orario: pd.DataFrame, target_window: int = 24, threshold: int = 18
) -> pd.DataFrame:
    """Aggiunge la variabile target: evento con >=threshold stazioni nelle prossime N ore."""

    logger.info(
        f"🎯 Generazione target (finestra {target_window}h, soglia {threshold} stazioni)..."
    )
    ...
    df_orario["max_energia_futura"] = (
        df_orario["energia_max"]
        .rolling(window=target_window, min_periods=1)
        .max()
        .shift(-target_window)
    )
    ...
    df_orario["Target_Allarme"] = (df_orario["max_energia_futura"] >= threshold).astype(
        int
    )

    # Conta quanti eventi futuri superano la soglia

    df_orario["future_events_above_threshold"] = (
        df_orario["energia_max"]
        .rolling(window=target_window, min_periods=1)
    ...
    logger.info(
        f"📊 Target generato: {df_orario['Target_Allarme'].sum()} allarmi su {len(df_orario)} ore"
    )
...
def add_spatial_features(
    ...
    try:
        ...
        logger.warning(
            "File output_eventi_georeferenziati.csv.gz non trovato, feature spaziali saltate"
        )
        return df_orario
    except DataValidationError as e:
    ...
    df_orario["distanza_da_centro_km"] = df_orario.apply(
        lambda row: (
            haversine_distance(
                LON_CENTRO, LAT_CENTRO, row["mean_longitude"], row["mean_latitude"]
            )
            if pd.notna(row["mean_longitude"])
            else 0.0
        ),
        axis=1,
    )
    ...
    df_orario["direzione_da_centro_deg"] = df_orario.apply(
        lambda row: (
            calculate_mean_direction(
                df_geo[df_geo["ora"] == row.name], LON_CENTRO, LAT_CENTRO
            )
            if row.name in df_geo["ora"].values
            else 0.0
        ),
        axis=1,
    )
...
def add_seismological_features(
    df_orario: pd.DataFrame, bvalue_window: int = 24
) -> pd.DataFrame:
    ...
    try:
        ...
        if not catalogo_path.exists():
            logger.warning(
                f"File {catalogo_path} non trovato, feature sismologiche saltate"
            )
            return df_orario

        df_catalogo = pd.read_csv(catalogo_path)

        df_catalogo["Tempo_Riferimento_ISO"] = pd.to_datetime(
            df_catalogo["Tempo_Riferimento_ISO"]
        )
        ...
        df_orario["bvalue_rolling_24h"] = (
            df_orario["bvalue_approx"].rolling(window=bvalue_window).mean()
        )

        df_orario["bvalue_std_24h"] = (
            df_orario["bvalue_approx"].rolling(window=bvalue_window).std()
        )
        ...
        logger.warning(
            "File catalogo_terremoti_unici.csv non trovato, feature sismologiche saltate"
        )
    except Exception as e:
...
def add_statistical_features(df_orario: pd.DataFrame) -> pd.DataFrame:
    ...
    for percentile in [25, 50, 75, 90, 95]:
        df_orario[f"energia_p{percentile}_24h"] = (
            df_orario["energia_max"].rolling(24).quantile(percentile / 100)
        )

    # 2. Skewness e Kurtosis

    df_orario["skewness_eventi_24h"] = (
        df_orario["numero_eventi"]
        .rolling(24, min_periods=1)
        .apply(lambda x: pd.Series(x).skew(), raw=True)
    )

    df_orario["kurtosis_eventi_24h"] = (
        df_orario["numero_eventi"]
        .rolling(24, min_periods=1)
        .apply(lambda x: pd.Series(x).kurtosis(), raw=True)
    )

    # 3. Entropia

    df_orario["entropy_24h"] = (
        df_orario["numero_eventi"]
        .rolling(24)
        .apply(
            lambda x: (
                -np.sum((x / x.sum()) * np.log2(x / x.sum() + 1e-10))
                if x.sum() > 0
                else 0
            ),
            raw=False,
...
def remove_missing_rows(
    df: pd.DataFrame, max_missing_pct: float = 50.0
) -> pd.DataFrame:
    """
    ...
    if removed > 0:
        ...
        logger.info(
            f"🧹 Rimosse {removed} righe ({removed_pct:.1f}%) con troppi valori mancanti"
        )
        # Warn if too many rows removed

        if removed_pct > 80:
            logger.warning(
                f"Attenzione: Più dell'80% delle righe rimosse ({removed_pct:.1f}%)"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Preparazione dati ML per analisi sismica')
    parser.add_argument('--input-csv', type=str, default='output_eventi_georeferenziati.csv.gz',
                        help='File di input con dati georeferenziati')
    parser.add_argument('--catalogo-csv', type=str, default='catalogo_terremoti_unici.csv',
                        help='File con catalogo eventi')
    parser.add_argument('--stations-csv', type=str, default='stations.csv',
                        help='File con coordinate stazioni')
    parser.add_argument('--output-dir', type=str, default='.',
                        help='Directory di output per il dataset ML')
    ...
    try:
        ...
        # 1. Caricamento dati
        df = load_data(args.catalogo_csv)
        ...
        try:
            ...
            if len(stations) == 0:
                ...
                # We need to load the georeferenced data from the specified input file
                geo_path = Path(args.input_csv)
                if geo_path.exists():
                    df_geo = pd.read_csv(geo_path)
                    df_geo["arrival_iso"] = pd.to_datetime(df_geo["arrival_iso"])
                    if "latitude" in df_geo.columns and "longitude" in df_geo.columns:
                    ...
                    logger.warning(f"Input file {args.input_csv} not found, spatial features skipped")
                    
            else:
                logger.warning(
                    "File stations.csv senza colonne coordinate, feature spaziali saltate"
                )
        except FileNotFoundError:
        ...
        missing_pct = (
            df_ml.isnull().sum().sum() / (len(df_ml) * len(df_ml.columns))
        ) * 100
        ...
        output_path = Path(args.output_dir) / "dataset_ml_sismico.csv"
        df_ml.to_csv(output_path, index=True, index_label="Tempo")
        ...
        print(
            f"Percentuale allarmi: {df_ml['Target_Allarme'].sum() / len(df_ml) * 100:.2f}%"
        )
        print(f"Feature totali: {len(df_ml.columns) - 1}")
        print(f"Valori mancanti: {missing_pct:.1f}%")
        print("-" * 60)
        print(f"✅ Dataset addestramento avanzato salvato: '{output_path}'")
