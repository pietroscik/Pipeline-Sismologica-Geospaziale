...
epicentri["Distanza_dal_Centro_KM"] = calcola_distanza(
    LAT_CENTRO, LON_CENTRO, epicentri["lat"], epicentri["lon"]
)
...
if dist_maggio.mean() > dist_gennaio.mean():
    ...
    print(
        f"⚠️ PROPAGAZIONE IN CORSO: A maggio gli epicentri si sono espansi verso l'esterno di mediamente {differenza*1000:.1f} metri!"
    )
else:
    print(
        "🔒 CLUSTERING CONCENTRATO: Lo sciame è rimasto localizzato nello stesso volume focale."
    )
