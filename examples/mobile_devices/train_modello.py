...
def load_data(dataset_path: str = "dataset_ml_sismico.csv") -> pd.DataFrame:
    ...
    if not dataset_path.is_absolute():
        ...
        if candidate.exists():
            ...
            # Prova la directory del modulo
            candidate = Path(__file__).resolve().parent / dataset_path
            if candidate.exists():
                dataset_path = candidate
...
def main(argv=None):
    ...
    parser.add_argument(
        "--dataset", type=str, default="dataset_ml_sismico.csv", help="Dataset di input"
    )
    parser.add_argument(
        "--model-output-dir",
        type=Path,
        default=Path("mobile/models"),
