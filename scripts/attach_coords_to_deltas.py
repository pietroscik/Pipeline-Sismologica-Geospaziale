...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="CSV con colonna station e valori delta/statistici.",
    )
    parser.add_argument(
        "--stations-csv",
        required=True,
        type=Path,
        help="CSV con station, latitude, longitude.",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="EPSG per la proiezione metrico.",
    )
    parser.add_argument(
        "--value-column",
        help=(
...
def normalize_delta_column(
    df: pd.DataFrame, requested_column: str | None
) -> pd.DataFrame:
    candidates = [
        "delta_seconds",
        "base_mean",
    ...
    if requested_column:
        if requested_column not in df.columns:
            raise SystemExit(
                f"Colonna richiesta non presente in delta CSV: {requested_column}"
            )
    ...
    if values.isna().any():
        raise SystemExit(
            f"La colonna {source_column} contiene valori delta non numerici o mancanti."
        )
...
def main() -> None:
    ...
    station_df = load_csv_with_checks(
        args.stations_csv, {"station", "latitude", "longitude"}
    ).copy()
    ...
    if missing.any():
        ...
        raise SystemExit(
            f"Coordinate mancanti per le stazioni: {', '.join(missing_codes)}"
        )

    transformer = pyproj.Transformer.from_crs(
        "EPSG:4326", f"EPSG:{args.epsg}", always_xy=True
    )
    x, y = transformer.transform(
        merged["longitude"].to_numpy(), merged["latitude"].to_numpy()
    )
    merged["x_m"] = x
    merged["y_m"] = y
