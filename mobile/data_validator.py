...
def validate_numeric_range(
    df: pd.DataFrame, col: str, min_val: float, max_val: float, allow_nulls: bool = False
) -> None:
...
def validate_file_size(
    path: Union[str, os.PathLike], min_size: float = 0, max_size: float = 1e9
) -> None:
    """Validate file size (in bytes). Default max: 1GB."""
    size = os.path.getsize(path)
    if size < min_size or size > max_size:
        raise DataValidationError(f"File size {size} bytes out of range [{min_size}, {max_size}]")
...
def validate_station_data(df: pd.DataFrame) -> None:
    if not isinstance(df, pd.DataFrame) or not {
        ...
        raise DataValidationError("Missing required station columns")
...
def validate_data(
    df: pd.DataFrame, required_columns: set, expected_types: Dict = None
