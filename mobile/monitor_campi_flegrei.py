...
from mobile.alert_system import AlertConfig, AlertMessage, AlertSystem
...
class CampiFlegreiMonitor:
    ...
    def __init__(
        ...
        self.config_path = (
            config_path or PROJECT_ROOT / "mobile/config/monitor_config.yaml"
        )
    ...
    def _load_config(self) -> dict:
        """Carica il file di configurazione monitor_config.yaml."""
        try:
            if self.config_path.exists():
                with open(self.config_path, 'r') as f:
                    return yaml.safe_load(f) or {}
            else:
                self.logger.warning(f"File config non trovato: {self.config_path}")
                return {}
    ...
    def _initialize_alert_system(self) -> None:
        ...
        try:
            ...
            if config_path.exists():
                ...
                config = AlertConfig.from_yaml(
                    str(PROJECT_ROOT / "mobile/config/alert_config.yaml")
                )
    ...
    def _get_stations_in_area(self) -> list:
        ...
        # Prova a prendere l'area dal config, altrimenti usa il default
        area_cfg = self.config_data.get("area", DEFAULT_AREA)
        lat = area_cfg.get("latitude", DEFAULT_AREA["latitude"])
        lon = area_cfg.get("longitude", DEFAULT_AREA["longitude"])
        radius = area_cfg.get("radius_km", DEFAULT_AREA["radius_km"])
        ...
        try:
            ...
            # FIX CRITICO 1: Stazioni lette dinamicamente dal file YAML (fallback sicuro)
            fallback_stations = self.config_data.get("stations", {}).get("default", [
                "CAAM", "CAFL", "CPOZ" # Fallback del fallback
            ])
            self.logger.warning(f"Using {len(fallback_stations)} fallback stations from config")
            return fallback_stations

    def _fetch_latest_waveforms(
        self, stations: list, minutes: int = 15
    ) -> Optional[pd.DataFrame]:
        """Scarica gli ultimi waveform per le stazioni specificate con batch processing."""
        from obspy import UTCDateTime
        from obspy.clients.fdsn import Client
        ...
        # FIX CRITICO 2: Controllo lista vuota
        if not stations:
            self.logger.warning("Lista stazioni vuota, uso fallback dal config.")
            stations = self.config_data.get("stations", {}).get("default", ["CAAM", "CAFL"])

        stations_to_process = stations[: self.min_stations]
        batch_size = 5  # Numero di stazioni da elaborare prima di chiamare il GC

        try:
            # FIX MEDIO 5: Elaborazione a batch per non saturare la RAM
            for i in range(0, len(stations_to_process), batch_size):
                ...
                for station in batch:
                    try:
                        ...
                        self.logger.warning(
                            f"Error fetching data for station {station}: {e}"
                        )
                        continue
                
                # Forza il garbage collector dopo ogni batch di tracce sismiche
                gc.collect()
    ...
    def _predict_risk(self, df: pd.DataFrame) -> float:
        ...
        if self.model is not None:
            try:
                features = df[
                    ["mean", "std", "min", "max", "amplitude_range", "hour", "minute"]
                ]
                risk_scores = self.model.predict(features)
                risk = float(np.mean(risk_scores))
                self.logger.info(
                    f"Predicted risk score (model {self.model_version}): {risk:.4f}"
                )
                return risk
            except Exception as e:
        ...
        # FIX CRITICO 3: Divisore non più hardcoded, preso dal config
        # Se non lo trova nel config, usa 1000000.0 (1e6) come default
        std_divisor = self.config_data.get("thresholds", {}).get("std_divisor", 1000000.0)
        
        std_normalized = df["std"].mean() / std_divisor
        amplitude_normalized = df["amplitude_range"].mean() / std_divisor
    ...
    def _check_thresholds(
        self, risk_score: float, stations_count: int
    ) -> tuple[bool, str]:
        """Verifica se le soglie sono superate."""
        alerts = []
        if stations_count < self.min_stations:
            alerts.append(
                f"Stazioni insufficienti: {stations_count}/{self.min_stations}"
            )

        if risk_score > self.risk_threshold:
            alerts.append(
                f"Soglia rischio superata: {risk_score:.4f}/{self.risk_threshold}"
            )
    ...
    def run_once(self) -> bool:
        ...
        try:
            ...
            if len(stations) < self.min_stations:
                self.logger.warning(
                    f"Only {len(stations)} stations available, need {self.min_stations}"
                )
            ...
            alert_required, alert_message = self._check_thresholds(
                risk_score, len(stations)
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Test mode (no alerts sent)"
    )
    parser.add_argument(
        "--daemon", action="store_true", help="Daemon mode (auto-restart on error)"
    )
    ...
    risk_threshold = float(
        os.getenv("MONITOR_RISK_THRESHOLD", str(args.risk_threshold))
    )
