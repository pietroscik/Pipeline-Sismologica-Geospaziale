...
class ModelVersionManager:
    ...
    def __init__(self, model_type: str = "default", base_dir: Optional[Path] = None, use_mlflow: bool = True):
        ...
        logger.info(f"ModelVersionManager initialized for {model_type} at {self.models_dir}")
    ...
    def _init_mlflow(self) -> None:
        ...
        try:
            ...
            experiment_name = f"CampiFlegrei_{self.model_type}"
    ...
    def load_model(self, version: Optional[str] = None) -> Tuple[Any, Dict[str, Any]]:
        ...
        if not version_path.exists():
            ...
            if version in all_versions:
                ...
                raise ValueError(f"Version {version} not found. Available: {all_versions}")
        ...
        for version_dir in sorted(self.models_dir.iterdir()):
            if version_dir.is_dir() and version_dir.name != "current":
                ...
                versions.append({
                    ...
                    "metadata": metadata,
                    "is_current": (self.models_dir / "current").resolve() == version_dir.resolve()
                })
    ...
    def compare_versions(self, version1: str, version2: str) -> Dict[str, Any]:
        ...
        comparison = {
            "version1": version1,
            "version2": version2,
            "metadata_diff": {k: (meta1.get(k), meta2.get(k)) for k in set(meta1.keys()) | set(meta2.keys())},
            "performance_diff": {k: (perf1.get(k), perf2.get(k)) for k in set(perf1.keys()) | set(perf2.keys())}
        }
        
        comparison["performance_change"] = {}
        for metric in set(perf1.keys()) & set(perf2.keys()):
            if isinstance(perf1[metric], (int, float)) and isinstance(perf2[metric], (int, float)):
                change = perf2[metric] - perf1[metric]
                pct_change = (change / perf1[metric] * 100) if perf1[metric] != 0 else 0
                is_improvement = change > 0 if metric in ["accuracy", "r2", "precision", "recall", "f1"] else change < 0
...
def save_model(model: Any, model_type: str = "default", metadata: Optional[Dict[str, Any]] = None,
                performance: Optional[Dict[str, float]] = None, version: Optional[str] = None) -> str:
...
def load_model(model_type: str = "default", version: Optional[str] = None) -> Tuple[Any, Dict[str, Any]]:
    """Carica un modello."""
    manager = get_model_manager(model_type)
