...
def save_config(
    config: Dict[str, Any], config_path: str = "mobile/config/alert_config.yaml"
) -> bool:
    """
...
def update_nested_config(
    config: Dict[str, Any], keys: List[str], value: Any
) -> Dict[str, Any]:
    """
...
def validate_config(
    config: Dict[str, Any], required_sections: List[str] = None
) -> Tuple[bool, List[str]]:
    """
...
def get_default_config() -> Dict[str, Any]:
    ...
    return {
        "alert": {
            "enabled": True,
            "min_stations": 15,
            "thresholds": {"low": 0.5, "medium": 0.7, "high": 0.8, "critical": 0.9},
            "cooldown_minutes": 60,
        },
        "notifications": {
            "email": {
