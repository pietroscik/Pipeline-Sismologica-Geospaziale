...
def resolve_input_path(path_str: Optional[str], fallback_dirs: List[Path]) -> Optional[Path]:
    """
...
def validate_csv_input(
    ...
    try:
        ...
        logger.error(f"Validazione fallita per {path_description} ({path}): {e.message}")
        for err in e.errors:
            logger.error(f"  - {err}")
        raise
...
"""
    ...
    try:
        ...
        if args.start_phase <= 0 and not args.skip_phase0:
            ...
            if stations_csv is None:
                ...
                try:
                    validate_csv_file(stations_csv, required_columns={"station", "latitude", "longitude"})
        ...
        if args.start_phase <= 2 and not args.skip_phase2:
            if delta_csv:
                ...
                if not out_station_deltas.exists():
                    ...
                    raise FileNotFoundError(f"Delta CSV file not found: {out_station_deltas}")
            else:
                ...
                if events_csv is None or picks_csv is None:
                    ...
                    raise ValueError("Events CSV and Picks CSV are required for Phase 2")

                # Validate input CSV files
                try:
                    validate_csv_file(events_csv, required_columns={"event_id", "time"})
                    validate_csv_file(picks_csv, required_columns={"pick_id", "event_id", "phase"})
            ...
            if delta_csv:
                ...
                # Validate delta CSV
                try:
                    validate_csv_file(delta_csv, required_columns={"station", "delta_seconds"})
        ...
        if args.start_phase <= 4 and not args.skip_phase4:
            ...
            if out_deltas_spatial is None or not out_deltas_spatial.exists():
                ...
                raise FileNotFoundError(f"Spatial delta file missing: {out_deltas_spatial}")
        ...
        if args.mobile_analysis and args.start_phase <= 4:
            ...
            if not mobile_script.exists():
                ...
                cmd_mobile = [
                    ...
                    "--output-dir", str(mobile_output_dir),
    ...
    # Import pandas for type hint (needed for mypy)
